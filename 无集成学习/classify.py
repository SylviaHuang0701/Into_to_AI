...
class RumourDetectClass:
    def __init__(self, model_path='transformer_rumor_detector.pt', data_path='./data/train.csv', vocab_path='vocab.json'):
        checkpoint = torch.load(model_path, map_location=DEVICE)
        train_df = pd.read_csv(data_path)
        with open(vocab_path, 'r') as f:
            self.vocab = json.load(f)
        num_events = train_df['event'].max() + 1
    ...
    def classify(self, text: str) -> int:
        ...
        with torch.no_grad():
            logits = self.model(text_ids, event_id)
